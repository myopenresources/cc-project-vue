<template>
          ...
          <a-input v-model:value="modelRef.accessUrl" placeholder="请输入...">
            <template #suffix>
              <a-tooltip title="从访问地址功能中导入已配置的地址">
                <span @click="openAccessUrlModal" class="app-cursor-pointer">
                  <app-dynamic-icon iconName="PlusOutlined" />
                </span>
              </a-tooltip>
            </template>
          </a-input>
  ...
  <app-access-url-modal
    v-if="accessUrlModalVisble"
    v-model:visible="accessUrlModalVisble"
    @selectItem="selectAccessUrl"
  />
</template>
<script lang="ts">
...
import AccessUrlModal from '/@/platform/components/access-url-modal/AccessUrlModal.vue';

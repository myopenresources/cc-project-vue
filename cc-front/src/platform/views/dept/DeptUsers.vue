<template>
  ...
  >
    <app-def-drawer-layout :title="'部门用户'" @close="handleCancel">
      <div class="app-dept-user-container">
...
import DeptType from '/@/platform/types/dept-type';
import DeptApi from '/@/platform/apis/dept-api';
...
export default defineComponent({
  ...
  setup(props, context) {
    const dataSource = shallowRef<DeptType[]>([]);

    /**
     * 初始化页面数据
     */
    const initPageData = () => {
      DeptApi.queryDeptUsers(props.id).then((res) => {
        if (HttpResultUtils.isSuccess(res)) {
          dataSource.value = res.data.resultData.deptUser;
        } else {
